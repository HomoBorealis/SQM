...
//полный код не влезает на ардуино нано, но можно убрать некоторые функции
...
//#define pinRX 6                          //  GPS Определяем вывод RX (программного UART) на плате Arduino к которому подключён вывод TX модуля. Номер вывода можно изменить.
//#define pinTX 5  
//время обновления
#define UpdateMeteo 30
#define UpdateGPS  20
...
volatile uint32_t debounce;//это для функции прерывания если чо
...
//GraphTemp();//проверить, может быть кривой
while (round(millis()/1000) % UpdateMeteo == 0);
}
  break;
  case 2:
{
  GraphPres();//проверить, может быть кривой
if (round(millis()/1000) % UpdateMeteo == 0)
  {
  ReadMeteo();
  GraphPres();
