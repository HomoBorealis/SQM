...
void ModeFunc()//функция прерывания для переключения режимов
{
  ...
  if (millis() - debounce >= 100 && digitalRead(3)) {//эта вся фигня для устранения дребезга контактов(лень паять кондёры)
    debounce = millis();
Mode++;
...
void ReadSQM() { 
  ...
  mag = mag + ReadEESqmCalOffset();//калибровка значения темноты неба (из EEPROM)
